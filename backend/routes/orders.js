  ...
  try {
    ...
    if (io) {
      ...
      io.to(`order_${id}`).emit("order-status-update", {
        ...
        verification_code: codeResult.rows[0]?.verification_code || null,
        timestamp: new Date(),
      });
    }
...
router.put("/driver/accept-package/:id", verifyToken, authorizeRoles("driver"), async (req, res) => {
  try {
    ...
    io.to(`order_${id}`).emit("order-status-update", {
      ...
      message: "A driver has been assigned to your package delivery! Check your verification code below."
    });
